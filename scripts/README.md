...
| 脚本 | 用途 | 依据 |
| --- | --- | --- |
| `check_knowledge_units.py` | 校验知识单元 front matter、编号唯一性、悬空引用、自环、循环依赖、**派生数据一致性** | SPEC-06 §4 §5 §5.6 |
...
| **派生数据漂移** | `_graph.json` 的节点/边/计数/`root_nodes`/`sink_nodes` 与源数据不一致 | ✅ |
| **漏边** | 实际存在依赖关系但未写入 `prerequisites` | ❌ **只能靠人工评审** |

> 倒数第二行对应 SPEC-06 §5.6 与 ADR-0001 §五。**这项校验是被一次真实事故逼出来的**：
> `_graph.json` 声明为「由脚本导出」，实际是手写的，`root_nodes` 被误填为全部 15 个节点
> （真实为 5 个），而 `_index.md` 又写作 4 个，三处数字互相矛盾，
> 且因当时不校验派生产物，这套错误在**校验全绿**的状态下被提交。
> 详见 `iteration-log/` 的 2026-09-24 追加记录。
...
- `scripts/export_graph.py` 待实现：从各知识单元的 `prerequisites` 读取边表，
  ...
  **优先级为最高** —— 本次事故（见上）的根因正是「声明由脚本派生，实际是手写」。
  在实现之前，`check_knowledge_units.py` 的派生数据一致性校验是唯一的防线。
